...
SUBROUTINE print_run_header(runs, nsim, date_time, folder, lene, simf, simi, iono)
   ...
   timpnufigrabit1 = REAL(Np, wp)*REAL(Nloop, wp)*REAL(Nreal, wp)*REAL(Nt, wp) &
                     *(1.2_WP + 0.7_WP*REAL(USE_coll, wp) + 3.9_WP*REAL(USE_turb, wp) &
                       + 1.5_WP*REAL(USE_coll, wp)*REAL(USE_turb, wp))/10.0**8
