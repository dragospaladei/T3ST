...
CONTAINS
   ...
   SUBROUTINE load_run_parameters(run)
      ...
      INTEGER :: ierr_sol
      REAL(KIND=wp) :: rpsi_ref, disc, w0_ref
      ...
      IF (magnetic_model == 2) THEN
         gama = 1.0_WP - 2.0_WP*(a0/R0)*sqrt(1.0_WP - (a0/R0)**2)
         alfa = elong*(sqrt(2.0_WP - gama) - sqrt(gama))/sqrt(2.0_WP*(1.0_WP - gama))

        rpsi_ref = sqrt(((X0/R0)*(X0/R0) - 1.0_WP)**2 + 4.0_WP*(Y0/R0)*(Y0/R0)*((X0/R0)*(X0/R0) - gama)/(alfa*alfa))/(1.0_WP - gama)
         w0_ref = solovev_w0_at_rpsi(rpsi_ref, gama)
         disc = safety*safety - 0.25_WP*gama*((1.0_WP - gama)**2)*rpsi_ref*rpsi_ref*w0_ref*w0_ref
   ...
   SUBROUTINE build_solovev_data(data_out, ierr)
      ...
      DO i = 1, Nsol - 2, 2
         ...
         Jnode(i + 1) = Jnode(i) + dr*(5.0_WP*fJ(i) + 8.0_WP*fJ(i + 1) - fJ(i + 2))/12.0_WP

         Jnode(i + 2) = Jnode(i) + dr*(fJ(i) + 4.0_WP*fJ(i + 1) + fJ(i + 2))/3.0_WP
