    ...
    SUBROUTINE Equil(X, Y, Z, Bx, By, Bz, gradBx, gradBy, gradBz, gradu2x, gradu2y, gradu2z,&
       ...
       REAL(KIND=wp), DIMENSION(Np), INTENT(OUT)  :: phi0x, phi0y, phi0z                                 ! The gradient of the neoclassical field Phi_0 (for quasineutrality in rotating plasmas)
       REAL(KIND=wp), DIMENSION(Np), INTENT(OUT)  :: gradBx, gradBy, gradBz, gradu2x, gradu2y, gradu2z   ! Grad B, grad u**2
       REAL(KIND=wp), DIMENSION(Np), INTENT(OUT)  :: rotbx, rotby, rotbz, rotux, rotuy, rotuz            ! curl(b), curl(u) components in the x,y,z system
       REAL(KIND=wp), DIMENSION(Np), INTENT(OUT)  :: Bx, By, Bz, B                                       ! Bx,  By, Bz, B components in the x,y,z system
       REAL(KIND=wp), DIMENSION(3, 3, Np), INTENT(OUT)  :: G                                                   ! G_ij = grad(qj).dr/dxi;
       REAL(KIND=wp), DIMENSION(Np), INTENT(OUT)  :: q1, q2, q3                                            ! Scaled field-aligned coordinates, a la GENE

       ! Local variables
       REAL(KIND=wp), DIMENSION(Np)                    :: hx, hy, hz                                          ! lame coefficients
       REAL(KIND=wp), DIMENSION(Np)                    :: psi, psir, psiz, psirr, psizz, psirz, rhot, rhotr, rhotz                ! miscelaneous
