...
SUBROUTINE Circular(npoints, X, Y, R)
   USE constants
   IMPLICIT NONE
   ...
   ! Locals
   !---------------------------------------------------------------------------------
   REAL(KIND=wp), DIMENSION(npoints) :: rr, theta, psiprim, psiprim2
   REAL(KIND=wp), DIMENSION(npoints) :: rhot, rhotr, rhotz, rhotrr, rhotrz, rhotzz
