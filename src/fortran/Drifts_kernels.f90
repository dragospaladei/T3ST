...
contains
   ...
   pure subroutine Drift2(dt, xi, yi, zi, vpi, mui, weighti, q1, q2, q3, time, &
                          vx, vy, vz, ap, vm, vw, vq, heat, Hi, Pc, B, Vtx, Vty, check_1, check_2, check_3, &
                          Qx1, Qy1, Qz1, Qw1, Qph1, anormal)
      !$omp declare simd(Drift2) uniform(dt,anormal,time,Qx1,Qy1,Qz1,Qw1,Qph1) notinbranch

      !---------------------------------------------------------------------------------
      ! Arguments
      !---------------------------------------------------------------------------------
      real(rp), intent(in)  :: dt, xi, yi, zi, vpi, mui, weighti, time, anormal
      real(rp), intent(out) :: q1, q2, q3, vx, vy, vz, ap, vm, vw, vq, heat, Hi, Pc, B, Vtx, Vty, check_1, check_2, check_3
...
! Apply the normalization once
     ...
     check_3 = phiy
      ...
      ! vW
      !---------------------------------------------------------------------------------
      heat = 0.5_rp*vpi*vpi + mui*B         ! note that this form is pure kinetic energy, without rotational or potential contributions       
      vq = vx_1*G(1,1) + vy_1*G(2,1) + vz_1*G(3,1)
