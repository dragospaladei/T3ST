...
class SimulationGUI:
    ...
    def build_layout(self):
        ...
        # Parameters (grouped) - shown as tabs (Notebook) to reduce visual crowding
        self.params_notebook = ttk.Notebook(self.scrollable_frame)
        self.params_notebook.grid(row=row, column=0, columnspan=UI_TOTAL_COLUMNS, sticky="nsew")
        row += 1

        # Ensure scrolling region updates when switching tabs
        self.params_notebook.bind("<<NotebookTabChanged>>", lambda *_: self.canvas.configure(scrollregion=self.canvas.bbox("all")))
        ...
        # Group parameters by p.group and create one tab per group.
        ...
        self.group_tabs: Dict[str, ttk.Frame] = {}

        for g in group_order:
            tab = ttk.Frame(self.params_notebook)
            self.params_notebook.add(tab, text=g)
            self.group_tabs[g] = tab

            # Configure columns (label, widget) pairs in this tab.
            ...
            for i, p in enumerate(params_in_group):
                ...
                lbl = ttk.Label(tab, text=label_text, style="Param.TLabel")
                ...
                if p.kind == "enum":
                    ...
                    cb = ttk.Combobox(tab, values=values, width=UI_COMBO_WIDTH, state="readonly")
                    ...
                    v = tk.IntVar(value=0)
                    chk = ttk.Checkbutton(tab, variable=v)
                    ...
                    ent = ttk.Entry(tab, width=UI_ENTRY_WIDTH, validate=validate, validatecommand=vcmd)
    ...
    def apply_filter(self):
        ...
        # Show/hide matching widgets across all tabs.
        ...
        # If filtering yields no visible fields on the current tab, jump to the first tab that has matches.
        if q and hasattr(self, "params_notebook") and hasattr(self, "group_tabs"):
            cur = self.params_notebook.select()
            has_visible_on_current = False
            for (lbl, _entry) in self.param_rows:
                if lbl.winfo_ismapped() and lbl.winfo_parent() == cur:
                    has_visible_on_current = True
                    break
            if not has_visible_on_current:
                for g, tab in self.group_tabs.items():
                    if tab.winfo_exists():
                        tab_id = tab._w  # tkinter internal widget path
                        for (lbl, _entry) in self.param_rows:
                            if lbl.winfo_ismapped() and lbl.winfo_parent() == tab_id:
                                self.params_notebook.select(tab)
                                return
