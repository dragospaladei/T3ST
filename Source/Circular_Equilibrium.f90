...
SUBROUTINE Circular(X, Y, R)
   ...
   ! Locals
   !---------------------------------------------------------------------------------
   REAL(KIND=rp), DIMENSION(Np) :: rr, theta, psiprim, psiprim2
   REAL(KIND=rp), DIMENSION(Np) :: rhot, rhotr, rhotz, rhotrr, rhotrz, rhotzz
   REAL(KIND=rp), DIMENSION(Np) :: psi, psir, psiz, psirr, psizz, psirz
   REAL(KIND=rp), DIMENSION(Np) :: chi, chir, chiz, qpsi, qprim, Fpsi, Fprim
   INTEGER :: i

   !---------------------------------------------------------------------------------
   ! Geometry / coordinates
   !---------------------------------------------------------------------------------
   rr    = sqrt((X - 1.0_rp)**2 + Y**2) + eps_rr
   ...
   rhot   = rr/a0
   rhotr  = (X - 1.0_rp)/rr/a0
   rhotz  = Y/rr/a0
   rhotrr = (1.0_rp/a0) * Y**2/rr**3
   rhotrz = (1.0_rp/a0) * Y*(1.0_rp - X)/rr**3
   rhotzz = (1.0_rp/a0) * (1.0_rp - X)**2/rr**3

   !---------------------------------------------------------------------------------
   ! q(r) and psi(r) + derivatives
   !---------------------------------------------------------------------------------
   qpsi  = s1 + s2*rhot + s3*rhot**2
   qprim = s2 + 2.0_rp*s3*rhot

   psiprim  = a0*rr/qpsi/sqrt(1.0_rp - rr**2)
   psiprim2 = -a0**2 * (rr/qpsi/sqrt(1.0_rp - rr**2)) * (qprim/a0/qpsi - 1.0_rp/(1.0_rp - rr**2)/rr)
   ...
   ! Closed-form psi(r)
   psi = -(a0**2/s3) / sqrt(1.0_rp + a0**2*s1/s3) * &
         ( atanh( sqrt(1.0_rp - rr**2) / sqrt(1.0_rp + a0**2*s1/s3) ) - &
           atanh( 1.0_rp / sqrt(1.0_rp + a0**2*s1/s3) ) )
...
SUBROUTINE psisurf_circ_old(X, Y, Vp)
   ...
   aux3 = a0*aux3
   aux  = pi*(2.0_rp*aux - 1.0_rp)     ! theta in [-pi, pi]
   ...
   DO k = 1, Np
      ...
      ioc = minloc(abs(psi/psi0 - real(USE_PC,rp)*rhoi/R0*As/Zs*F/normB*Vp(k)/psi0 - 1.0), 1)
...
SUBROUTINE psisurf_circ(X, Y, Vp)
   ...
   ! I/O
   !---------------------------------------------------------------------------------
   REAL(rp), INTENT(OUT) :: X(Np), Y(Np)
   REAL(rp), INTENT(IN)  :: Vp(Np)
   ...
   REAL(rp), SAVE :: r_g(Nr), inv_base_norm(Nr), psi_over_psi0(Nr)
!   LOGICAL, SAVE  :: inited = .false.
 ...
 !  IF (.NOT. inited) THEN
  ...
  ! END IF
   ...
   th = pi*(2.0_rp*th - 1.0_rp)

   alpha = real(USE_PC,rp)*rhoi/R0*As/Zs/psi0   ! F=1 ⇒ K/psi0
   ...
   DO k = 1, Np
      ...
      DO j = 1, Nr
         val = abs(psi_over_psi0(j) - alpha*Vp(k)*(1.0_rp + s*r_g(j))*inv_base_norm(j) - 1.0_rp)
         IF (val < best) THEN
            best = val
            ioc  = j
         END IF
      ...
      ! Map (r, theta) -> (X, Y)
      X(k) = 1.0_rp + r_g(ioc)*c
      Y(k) = r_g(ioc)*s


      ! Safety clamp (should never trigger since r_g ∈ [0, a0])
      IF ((X(k) - 1.0_rp)*(X(k) - 1.0_rp) + Y(k)*Y(k) > a0*a0) THEN
         X(k) = X0
         Y(k) = Y0
      END IF
   END DO
...
CONTAINS
   ...
   SUBROUTINE init_tables()
      ...
      INTEGER  :: j
      REAL(rp) :: r, rh, inv_a0, qpsi, psip, one_m_r2
      REAL(rp) :: inv_sqrt_beta, cA, cB, beta

      inv_a0 = 1.0_rp/a0

      ! Use mirpoints to avoid exactly r=0 and r=a0
      DO j = 1, Nr
         r      = (REAL(j, rp) - 0.5_rp) * a0 / REAL(Nr, rp)
         r_g(j) = r
      END DO

      beta          = 1.0_rp + (a0*a0)*s1/s3
      inv_sqrt_beta = 1.0_rp / sqrt(beta)
      ...
      DO j = 1, Nr
         ...
         one_m_r2 = max(1.0_rp - r*r, 1.0e-15_rp)
         psip     = r/(qpsi*sqrt(one_m_r2))                    ! psi'(r)

         inv_base_norm(j) = 1.0_rp / sqrt(1.0_rp + psip*psip)
         ! psi(r)/psi0
